...
class TestBaseAgent:
    ...
    @pytest.mark.asyncio
    async def test_analyze_with_structure_handles_no_response(self):
        class DummyAgent(BaseAgent):
            async def think(self, prompt: str, system_prompt: str = None):
                return None

        agent = DummyAgent("Test Agent")
        result = await agent.analyze_with_structure(
            "Test prompt",
            {"test_field": "string"},
        )

        assert isinstance(result, dict)
        assert result.get("error") == "No response from model"
...
class TestAgentIntegration:
    ...
    @pytest.mark.skipif(
        not __import__("os").getenv("OPENAI_API_KEY"), reason="OPENAI_API_KEY not set"
    )
    @pytest.mark.asyncio
